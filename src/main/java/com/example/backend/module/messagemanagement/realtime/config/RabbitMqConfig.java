 ...
 *
 * Exchanges:
 *   zunochat.direct   → mensajes, read-receipts
 *   zunochat.fanout   → presencia (broadcast a todos los consumers)
 *
 * Queues:
 *   q.messages        → nuevos mensajes de chat
 *   q.read.receipts   → confirmaciones de lectura
 *   q.presence        → eventos de presencia online/offline
 *
 * Dead-Letter:
 *   q.dead.letter     → mensajes fallidos tras reintentos
 */
@Configuration
public class RabbitMqConfig {
    ...
    @Bean
    public Queue readReceiptsQueue() {
    ...
    public Queue presenceQueue() {
        return QueueBuilder.durable(QUEUE_PRESENCE)
                .withArgument("x-message-ttl", 30_000)  // 30s TTL — presencia es efímera
                .build();
    }
    ...
    @Bean
    public Binding bindMessages(Queue messagesQueue, DirectExchange directExchange) {
        return BindingBuilder.bind(messagesQueue).to(directExchange).with(RK_MESSAGE);
    }

    @Bean
    public Binding bindReadReceipts(Queue readReceiptsQueue, DirectExchange directExchange) {
        return BindingBuilder.bind(readReceiptsQueue).to(directExchange).with(RK_READ);
    }

    @Bean
    public Binding bindPresence(Queue presenceQueue, FanoutExchange fanoutExchange) {
    ...
    public RabbitTemplate rabbitTemplate(ConnectionFactory factory) {
        ...
        template.setConfirmCallback((correlationData, ack, cause) -> {
            if (!ack) {
                // En producción: loguear o reencolar
                System.err.println("RabbitMQ: mensaje no confirmado — " + cause);
            }
        });
        return template;
    }
